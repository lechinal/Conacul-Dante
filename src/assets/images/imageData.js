import barman from "../../assets/images/barman.jpg";
import botezatu from "../../assets/images/botezatu.jpg";
import fructe from "../../assets/images/fructe.jpg";
import sampanie from "../../assets/images/sampanie.jpg";
import prajitura from "../../assets/images/prajitura.jpg";
import protap from "../../assets/images/protap.jpg";

import tm1 from "../../assets/images/tm1.jpg";
import tm2 from "../../assets/images/tm2.jpg";
import tm3 from "../../assets/images/tm3.jpg";
import tm4 from "../../assets/images/tm4.jpg";
import tm5 from "../../assets/images/tm5.jpg";
import tm6 from "../../assets/images/tm6.jpg";
import tm7 from "../../assets/images/tm7.jpg";

import ev1 from "../../assets/images/ev1.jpg";
import ev2 from "../../assets/images/ev2.jpg";
import ev3 from "../../assets/images/ev3.jpg";
import ev4 from "../../assets/images/ev4.jpg";

import room1 from "../images/room1.jpg";
import room2 from "../images/room2.jpg";
import room3 from "../images/room3.jpg";
import room4 from "../images/room4.jpg";

export const eventImages = [
  barman,
  botezatu,
  fructe,
  sampanie,
  prajitura,
  protap,
];

export const teamBuildingImages = [tm1, tm2, tm3, tm4, tm5, tm6, tm7];

export const evImages = [ev1, ev2, ev3, ev4];

export const roomsImages = [room1, room2, room3, room4];
